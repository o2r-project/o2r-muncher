...
module.exports.getGenerationFunction = function (file_name, callback) {
  ...
  switch (ext) {
    ...
    case '.rdata':
        ...
        f.fName = 'generateManifestForSessionInfoRData';
        callback(null, f);
        break;
    default:
...
module.exports.generateManifestForRMarkdown = function (job_id, payload_dir, main_file, display_file, update, callback) {
  ...
  // add label with generator metadata
  r_label_command = 'Label('
    + 'name = \'muncher\'' + ', '
    + 'version = \'' + config.version + '\', ' 
    + 'image = \'' + config.containerit.image + '\', '
    + 'timestamp = \'' + new Date().valueOf() + '\', '
    + 'label_ns = \'' + config.containerit.labelNamespace + 'generate.\')';

  // combine commands
  r_command = 'setwd(\'' + path_to_workdir_in_container + '\'); ' 
    + 'the_dockerfile <- ' + r_dockerfile_command + '; '
    + 'addInstruction(the_dockerfile) <- ' + r_label_command + '; '
  ...
  runContainerit(job_id, create_options, start_options, cmd, update, (err, result) => {
    if (err) {
      ...
      handleManifestFileOwnership(job_id, payload_dir, main_file, path_to_workdir_in_container,
        create_options, start_options, result, callback);
    }
  });
}
...
handleManifestFileOwnership = function(job_id, payload_dir, main_file, path_to_workdir_in_container,
                                       create_options, start_options, result, callback) {
  ...
  if(statsMainfile.uid != statsManifest.uid) {
    ...
    debug('[%s] No ownership mismatch between manifest file (%s) and main file (%s) - can continue!', job_id, statsManifest.uid, statsMainfile.uid);
    callback(null, result);
  }
}