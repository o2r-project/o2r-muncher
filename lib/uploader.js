...
var clam = require('clamscan')(config.bagtainer.scan.settings);
        ...
        return new Promise((fulfill, reject) => {
            if (!config.bagtainer.scan.enable) {
                ...
                clam.scan_dir(passon.bagpath, (error, good, bad) => {
                    if (error) {
                        debug(error);
                        passon.res.status(500).send(JSON.stringify({ error: 'virus scan failue: ' + error.message }));
                        ...
                        if (bad.length > 0) {
                            ...
                            passon.res.status(422).send(JSON.stringify({ error: 'virus scan found infected file(s) in directory' }));
                        } else {
