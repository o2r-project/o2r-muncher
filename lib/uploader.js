    ...
    this.extractMetadata = (passon) => {
        return new Promise((fulfill, reject) => {
            ...
            logStream._write = function (chunk, enc, next) {
                debug('[o2r-meta-extract] [%s] %s', this.compendium_id, chunk);
                next();
            }
            ...
            create_options.HostConfig.Binds = [
                ...
                + config.bagtainer.metaextract.mountpoint + ':rw'
            ];
            let cmd = ['-i', config.bagtainer.metaextract.mountpoint,
                '-o', path.join(config.bagtainer.metaextract.mountpoint, config.bagtainer.metaextract.outputDir)];

            debug('[%s] Running container with command "%s" and options: %s', cmd, JSON.stringify(create_options));
