...
if (config.email.enable
    ...
    debug('Email notification for virus detection _not_ active: %s', JSON.stringify(config.email));
}
...
function cleanup(passon) {
    return new Promise((fulfill, reject) => {
        ...
        if (passon.metaextract_container_id) {
            debug('[%s] Deleting metadata extraction container %s', passon.id, passon.metaextract_container_id);
