...
module.exports.broker = function (id, metadata_dir, metadata_file, current_mapping) {
    return new Promise((fulfill, reject) => {
        debug('[%s] Brokering "%s" metadata using file %s to directory %s', id, current_mapping, metadata_file, metadata_dir);

        let binds = [
            metadata_dir + ':' + metadata_dir
        ];
        if(config.fs.volume) {
            debug('[%s] volume is configured, overwritten binds configuration: %s', JSON.stringify(binds));
