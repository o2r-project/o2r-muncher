...
module.exports.addToDns = function (entityId, priority) {
    let instance = null;
    if (priority === config.dns.priority.journal)
        instance = Journal;
    else
        instance = Publisher;

    instance.findOne({id: entityId}, (err, entity) => {
        if (err || !entity) {
            debug('[%s] Could not find owner: %O', entityId, err);
            return;
        }

        Dns.findOne({'domains': entity.domains, priority: priority}, (err, dns) => {
            if (err) {
                debug('[%s] Could not find DNS Server for owner: %O', entityId, err);
            } else {
                if (!dns) {
                    debug('[%s] No existing DNS Server for this configuration, creating new one');
                    createDnsServer(entity.id, entity.domains, priority);
                } else {
                    debug('[%s] Found DNS Server for owner: %s', entityId, dns.id);
                    dns.updateOne({$addToSet: {owner: entityId}}, (err, result) => {
                        if (err)
                            debug('[%s] Could not add owner to dns server configuration: %O', entityId, result);
                        else {
                            debug('[%s] Added owner to dns server configuration: %O', entityId, result);
                        }
                    })
                }
            }
...
module.exports.removeJournalFromDns = function (entityId, priority) {
    let instance = null;
    if (priority === config.dns.priority.journal)
        instance = Journal;
    else
    ...
    instance.findOne({id: entityId}, (err, entity) => {
        ...
        Dns.findOne({owner: entity.id}, (err, dns) => {
            if (err) {
                debug('[%s] Could not find DNS Server for owner: %O', entityId, err);
            } else {
                dns.owner.splice(dns.owner.indexOf(entity.id), 1);
                if (dns.owner.length < 1) {
                    ...
                    dns.save(err => {
                        if (err)
                            ...
                            debug('[%s] Deleted owner %s from dns server', dns.id, entityId);
                        }
                    });
                }
            }
...
createDnsServer = function (entityId, entityDomains, priority) {
    ...
    return new Promise((fulfill, reject) => {
        ...
        Dns.findOne({id: dnsId}, (err, dns) => {
            if (err) {
                ...
                addDomainsToDns(dnsConfigPath + '/dnsmasq.conf', dns.domains)
                    .then(() => {
