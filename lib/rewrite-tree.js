...
// extra mime types with a fallback option
function extraMimeTypes(extension) {
  let theType = extraTypes[extension.toLowerCase()];
  return((theType === undefined) ? 'text/plain' : theType);
}
...
function rewriteTree(tree, trunc, newpath) {
  ...
  if (tree.children) {
    ...
    tree.type = mime.lookup(tree.path) || extraMimeTypes(tree.extension);
    return finish(tree);
  }
}
