          ...
          this.updateStep('generate_configuration', 'running', 'configuration file not found, generating it...', (err) => {
            if (err) reject(err);
              ...
              var configuration = {
                ...
                // get main and display from the compendium
      ...
      this.updateStep('generate_manifest', 'running', null, (err) => {
        if (err) reject(err);
          ...
          fs.open(manifestFile, 'r+', (error) => {
            if (error) {
              debug('[%s] Run analysis and write manifest to %s', jid, payloadDir);
                      ...
                      if (err) {
                        ...
                        fs.open(manifestFile, 'r+', (error) => {
                          if (error) {
                            ...
                            debug('[%s] Created manifest at %s: %o', jid, manifestFile, done);
                                  ...
                                  if (err) reject(err);
                                    ...
                                    Job.findOne({ id: jid }, (err, job) => {
                                      if (err) reject(err);
                                        ...
                                        manifestFilePath = path.relative(payloadDir, manifestFile);
  ...
  this.prepareImage = (passon) => {
    return new Promise((fulfill, reject) => {
      this.updateStep('image_prepare', 'running', null, (err) => {
        ...
        debug('[%s] Packing payload for Docker execution using manifest at %s', this.jobId, passon.manifestFile);
          ...
          if (err) {
            ...
            fs.access(imageTarballFile, fs.constants.F_OK, (err) => {
              if (err) {
                ...
                debug('[%s] Loading image tarball from file %s', this.jobId, imageTarballFile);
                stepUpdate('image_build', null, 'Image tarball found! Loading it...', (err) => {
                  ...
                  docker.loadImage(tarStream, {}, (error, stream) => {
                    ...
                    stream.on('data', function (d) {
                      debug('[%s] [image loading] %o', d);
                    });

                    stream.on('end', function () {
                      debug('[%s] image loaded from tarball.');

                      // TODO add additional tags to the loaded image, it might not have the job tag!
                      passon.imageTag = imageTag;
                      fulfill(passon);
                    });
                  });
            ...
            stepUpdate('image_build', null, 'Found image for compendium: ' + imageTagCompendium, (err) => {
              if (err) reject(err);
  ...
  this.executeImage = (passon) => {
    return new Promise((fulfill, reject) => {
      ...
      this.updateStep('image_execute', 'running', '[started image execution]', (err) => {
        ...
        let binds = [
          payload_path + ':' + config.bagtainer.mountLocationInContainer
        ];
        ...
        if (passon.configuration.execution && passon.configuration.execution.bind_mounts) {
          passon.configuration.execution.bind_mounts.forEach(bm => {
            bmString = path.join(payload_path, bm.source) + ":" + bm.destination;
                  ...
                  if (err) {
                    ...
                    stepUpdate('check', passon.check.status, passon.check.text, (error) => {
                      if (error) {
                        ...
                        let imageTag = config.bagtainer.image.prefix.compendium + compendium_id;
                        debug('[%s] Tagging image for compendium after successful execution: ', job_id, imageTag);
                        let image = docker.getImage(passon.imageTag);
                        image.tag({
                          repo: config.bagtainer.image.name.compendium,
                          tag: compendium_id
                        }, (err, data) => {
                          if (err) {
                            debug('[%s] Error during tagging of image with compendium ID: %o', job_id, err);
                            reject(err);

                          } else {
                            // this is the final fulfill
                            fulfill(passon);
                          }
                        });
                      }
          ...
          if (passon.check.status === "success" && config.bagtainer.saveImageTarball) {
            ...
            fs.access(imageTarballFile, fs.constants.F_OK, (err) => {
              if (err) {
                ...
                debug('[%s] image tarball file already exists, skipping saving to compendium.', job_id, imageTarballFile);
                stepUpdate('image_save', 'skipped', '[Image tarball file saving skipped because file already exists]', (error) => {
                  if (error) reject(error);
                  else fulfill(passon);
                });
              }
