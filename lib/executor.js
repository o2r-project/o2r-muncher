  ...
  this.createOrLoadConfigurationFile = (passon) => {
    return new Promise((fulfill, reject) => {
      this.updateStep('generate_configuration', 'running', null, (err) => {
        ...
        if (this.compendium.bag) {
          configFilePathJob = path.join(passon.jobPath, config.bagit.payloadDirectory, config.bagtainer.configFile.name);
        } else {
          configFilePathJob = path.join(passon.jobPath, config.bagtainer.configFile.name);
        }
