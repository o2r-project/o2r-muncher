...
function Executor(jobId, compendium) {
  ...
  this.updateStep = (step, status, text, callback) => {
    ...
    let update = {};
    if (status) {
      update['$set'] = { ['steps.' + step + '.status']: status };

      let d = new Date();
      switch (status) {
        case 'skipped':
          update['$set'] = { ['steps.' + step + '.start']: d };
          update['$set'] = { ['steps.' + step + '.end']: d };
          break;
        case 'running':
          update['$set'] = { ['steps.' + step + '.start']: d };
          break;
    ...
    if (text) {
      if (isArray(text)) {
        texts = text.map(t => t.toString());
        update['$push'] = { ['steps.' + step + '.text']: { '$each': texts } };
      } else update['$push'] = { ['steps.' + step + '.text']: text };
    }
            ...
            { t: imageTag }, (error, output) => {
              if (error) {
                ...
                output.on('data', d => {
                  ...
                  stepUpdate('image_build', null, lastData.stream.trim(), (err) => {
                    if (err) {
                ...
                output.on('end', () => {
                  ...
                  else if (lastData.stream && (lastData.stream.startsWith('Successfully tagged') || lastData.stream.startsWith('Successfully tagged'))) {
    ...
    return new Promise((fulfill, reject) => {
      ...
      this.updateStep('image_execute', 'running', 'started image execution', (err) => {
        ...
        stdStream._write = function (chunk, enc, next) {
          ...
          debugRun('[%s] [run] %s', this.job_id, msg);
        ...
        docker.run(passon.imageTag, [], stdStream, create_options, start_options, (err, data, container) => {
          ...
          if (err) {
            stepUpdate('image_execute', 'failure', JSON.stringify(err), (error) => {
            ...
            if (data.StatusCode === 0) {
              fields = {};
              fields['steps.image_execute'] = {
                'statuscode': data.StatusCode,
                'status': 'success'
              };

              Job.updateOne({ id: job_id }, { $set: fields }, (error) => {
                if (error) reject(error);
        ...
        if (error) {
          ...
          checker(checkerConfig)
            .then(function (resolveMetadata) {
              ...
              if (checkerConfig.saveDiffHTML) {
                passon.check.display = {};
                passon.check.display.diff = config.api.resource.job
                  + job_id
                  + config.api.sub_resource.data
