...
const getDigestFromTarball = require('../lib/image').getDigestFromTarball;
      ...
      this.updateStep('generate_manifest', 'running', null, (err) => {
        if (err) reject(err);
          ...
          fs.open(manifestFile, 'r+', (error) => {
            if (error) {
              debug('[%s] Run analysis and write manifest to %s', job_id, payloadDir);
                      ...
                      if (err) {
                        ...
                        fs.open(manifestFile, 'r+', (error) => {
                          if (error) {
                            ...
                            copyFileFromJobToCompendium(config.bagtainer.manifestFile, job_id, cid, isBag, (err) => {
                              if (err) reject(err);
                              else {
                                    ...
                                    saveLog(cid, job_id, 'generate_manifest', isBag, stepUpdate, (errSaveLog) => {
                                      if (errSaveLog) reject(errSaveLog);
                                        ...
                                        Job.updateOne({ id: job_id }, { $set: fields }, (err) => {
                                          if (err) {
                                            debug('Error during manifest step update: %o', err.message);
                                            reject(err);
                                          } else {
                                            stepUpdate('generate_manifest', 'success', 'generated manifest', (error) => {
                                              if (error) {
                              ...
                              if (lastData.error) {
                                ...
                                saveLog(compendium_id, job_id, 'image_build', isBag, stepUpdate, (errSaveLog) => {
                                  if (errSaveLog) reject(errSaveLog);
                                    ...
                                    Job.findOne({ id: job_id }).select('steps').lean().exec((err, job) => {
                                      if (err) {
                                        debug('Error getting job to retrieve image digest: %o', err);
                                        reject(err);
                                      } else {
                                        if (job === null) {
                                          reject(new Error('[no job found with id ' + job_id + ']'));
                                        }
                                        else {
                                          buildLog = job.steps.image_build.text.filter(item => item.includes('sha256'));
                                          if (buildLog[0]) {
                                            digest = buildLog[0].match(/sha256:[a-z0-9]+/g)[0];
                                            debug('[%s] Extracted image digest %s from log %o"', job_id, digest, buildLog);
                              ...
                              (err) => {
                                ...
                                synchroniseImageTags(compendium_id, job_id, (err) => {
                                  if (err) reject(err);
                                    ...
                                    stepUpdate('image_build', 'skipped',
                                      'Loaded image tarball from file ' + config.bagtainer.imageTarballFile, (err) => {
                                        ...
                                        getDigestFromTarball(imageTarballFile, (er, digest) => {
                                          if(er) reject(er);
                      ...
                      if (digest != passon.imageId) {
                        stepUpdate('image_save', null, '[Image tarball updated because image digests mismatch: job: '
                          + passon.imageId + ' | tarball:' + tarballDigest + ']', (error) => {
                            ...
                            saveImageFromJob(job_id, imageTarballFile, stepUpdate, (err) => { // FIXME code duplicated above
                              if (err) {
