...
function Executor(jobId, basePath) {
  ...
  debug('[%s] Job set up with path %s', this.jobId, this.basePath + this.jobId);
  ...
  this.runContainer = (passon) => {
    return new Promise((fulfill, reject) => {
      ...
      this.updateStep('image_execute', 'running', null, (err) => {
        ...
        docker.run(passon.imageTag, [], stdStream, (err, data, container) => {
            ...
            new Promise((fulfill, reject) => {
              if (passon.container && !config.bagtainer.keepContainers) {
                ...
                passon.container.remove(function (err, data) {
                  ...
                  if (data === '') {
                    fulfill('Removed container ' + JSON.stringify(data));
                  } else {
                    reject(new Error('Error removing container: ' + data));
                  }
                });
              }
            }),
            new Promise((fulfill, reject) => {
              if (passon.container && !config.bagtainer.keepImages) {
                ...
                locateImageByTag(passon.imageTag, function (err, image) {
                  ...
                  function callback(error, data) {
                    ...
                    fulfill('Removed image for tag ' + passon.imageTag + ': ' + JSON.stringify(data));
                  }
  ...
  this.execute = () => {
    ...
    return this.loadBag({})
      ...
      .catch(res => {
        debug("[%s] Unhandled failure (or rejection) during cleanups: \n\t%s", this.jobId, res);
      });
  };
}
