...
const debugBuild = require('debug')('executor:build');
const debugRun = require('debug')('executor:run');
...
function Executor(jobId, compendium) {
   ...
   */
  this.textAppend = (step, data, cb) => {
    Job.findOne({ id: this.jobId }, (err, job) => {
      if (err) throw err;
      if (job === null) {
        cb(new Error('no job found with id ' + this.jobId));
      }
      else {
        // check if field exists, otherwise text field will include 'undefined'.
  ...
  this.updateStatus = (status, cb) => {
    ...
    Job.update({ id: this.jobId }, { $set: { status: status } }, (err) => {
      if (err) cb(err);
      else cb();
    });
  }
  ...
  this.copyFiles = (passon) => {
    return new Promise((fulfill, reject) => {
      ...
      debug('[%s] Copying job files to %s from %s', this.jobId, job_path, compendium_path);
            ...
            .then(res => {
              ...
              this.updateStep('validate_bag', 'failure', e.message, (err) => {
                ...
                if (config.bagit.failOnValidationError.execute) {
                  ...
                  debug('[%s] Bag invalid but _not_ failing this step. The error was: %s', this.jobId, e);
                  fulfill(passon);
                }
              });
            });
  ...
  copyFileFromJobToCompendium = function (file, job_id, compendium_id, isBag, callback) {
    ...
    try {
      ...
      fse.copySync(filePathJob, filePathCompendium);
      callback();
    } catch (error) {
      debug('[%s] Error copying file: %s', job_id, error);
      this.updateStep('generate_configuration', 'failure', 'error copying configuration file to compendium directory', (err) => {
        if (err) callback(err);
        else callback(error);
      });
    }
      ...
      if (config.bagtainer.validateCompendiumBeforeExecute) {
        ...
        this.updateStep('validate_compendium', 'running', null, (err) => {
          ...
          if (valid) { // config file is valid!
            ...
            debug('Configuration file is INvalid, but do not reject it.');
      ...
      this.updateStep('generate_manifest', 'running', null, (err) => {
        if (err) reject(err);
          ...
          fs.open(manifestFile, 'r+', (error) => {
            if (error) {
              debug('[%s] Run analysis and write manifest to %s', this.jobId, manifestFile);

              // TODO use containerit

              let isBag = this.compendium.bag, cid = this.compendium.id, jid = this.jobId;
              let stepUpdate = this.updateStep;
              fs.writeFile(manifestFile, 'FROM ubuntu\nCMD ["uname", "-a" ]', (err) => {
                if (err) reject(err);
                else {
                  // copy it to compendium directory
                  copyFileFromJobToCompendium(config.bagtainer.manifestFile, jid, cid, isBag, (err) => {
                    if (err) reject(err);
                      ...
                      stepUpdate('generate_manifest', 'success', 'created manifest file', (err) => {
                        if (err) reject(err);
                        else {
              ...
              debug('[%s] manifest file %s already present: %s', this.jobId, manifestFile, error);
            ...
            if (error) {
              ...
              output.on('end', () => {
                ...
                if (lastData.error) {
                  ...
                  debug('[%s] Created Docker image "%s", last log was "%s"', this.jobId, imageTag, lastData.stream);
    ...
    return new Promise((fulfill, reject) => {
      ...
      this.updateStep('image_execute', 'running', null, (err) => {
        ...
        stdStream._write = function (chunk, enc, next) {
          Job.findOne({ id: currentExecutor.jobId }, (err, job) => {
            if (err) reject(err);

            if (job === null) {
              reject(new Error('job ' + currentExecutor.jobId + ' not found'));
            }
            else {
              ...
              debugRun('[%s] [run] %s', currentExecutor.jobId, chunk);
          ...
          if (err) {
            ...
            if (data.StatusCode === 0) {
              ...
              container.logs({
                ...
                if (err)
                  debug('[%s] Error getting container logs after non-zero status code', );
                else {
  ...
  /*
   *  Run checker, see https://github.com/o2r-project/erc-checker/#current-functional-scope
   */
  this.check = (passon) => {
    return new Promise((fulfill, reject) => {
      ...
      if (!passon.configuration.display) {
        ...
        reject(new Error('Display file property missing, cannot run check'));
        return;
      }
      ...
      var checkerConfig = {
        ...
        pathToOriginalHTML: path.join(passon.compendiumPath, passon.configuration.display),
        pathToReproducedHTML: path.join(passon.jobPath, passon.configuration.display),
        saveFilesOutputPath: passon.jobPath,
        saveDiffHTML: true,
        ...
        if (error) {
          ...
          checker(checkerConfig)
            .then(function (resolveMetadata) {
              debug('[%s] Check succeeded: %s', this.jobId, resolveMetadata.checkSuccessful);
              passon.check = resolveMetadata;

              stepUpdate('check', 'success', 'Check successful', (error) => {
                if (error) {
                  debug('Error during check step update: %s', error.message);
                  reject(error);
                } else {
                  ...
                  fields['steps.check.diffHTML'] = passon.check.resultHTML;
                  fields['steps.check.errors'] = passon.check.errorsEncountered;

                  Job.update({ id: this.jobId }, fields, (err) => {
                    if (err) {
                      debug('Error during check step fields: %s', err.message);
                      reject(err);
                    } else {
                      fulfill(passon);
                    }
              ...
              debug('[%s] Check failed: %s', this.jobId, JSON.stringify(rejectMetadata));
              passon.check = rejectMetadata;

              this.updateStep('check', 'failure', JSON.stringify(rejectMetadata.errorsEncountered), (error) => {
                if (error) {
                  debug('Error during check step update: %s', error.message);
                  reject(error);
                } else {
  ...
  this.cleanup = (passon) => {
    return new Promise((fulfill, reject) => {
      debug('[%s] Run cleanup regularly with passon "%s"', this.jobId, JSON.stringify(passon));
      ...
      this.updateStep('cleanup', 'running', 'Running regular cleanup', (error) => {
        if (error) {
          ...
          if (passon) {
            Promise.all([
              // no overall status check needed here, end function should have been called
            ...
            this.updateStep('cleanup', 'success', 'Nothing provided that could be cleaned up', (err) => {
              if (err) {
                ...
                finalOverallStatusCheck(this.jobId, passon, fulfill, reject, this.updateStatus);
              }
            });
          }
        }
