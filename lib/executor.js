    ...
    return new Promise((fulfill, reject) => {
      if (config.bagtainer.validateBagBeforeExecute) {
        ...
        this.updateStep('validate_bag', 'running', null, (err) => {
          ...
          bag.validate(config.bagtainer.bagit.validateFast)
            .then(res => {
              ...
              this.updateStep('validate_bag', 'failure', e.message, (err) => {
                ...
                if (config.bagtainer.bagit.failOnValidationError.execute) {
    ...
    return new Promise((fulfill, reject) => {
      if (config.bagtainer.validateCompendiumBeforeExecute) {
        ...
        this.updateStep('validate_compendium', 'running', null, (err) => {
          if (err) reject(err);
          try {
            let configFilePath = path.join(this.basePath, this.jobId, config.bagtainer.payloadDirectory, config.bagtainer.configFile);
