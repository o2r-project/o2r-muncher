...
function Executor(jobId, compendium) {
  ...
  this.loadOrBuildImage = (passon) => {
    return new Promise((fulfill, reject) => {
      ...
      job_id = this.jobId;
      compendium_id = this.compendium.id;
        ...
        docker.run(passon.imageTag, [], stdStream, create_options, start_options, (err, data, container) => {
          ...
          if (err) {
            ...
            if (data.StatusCode === 0) {
              ...
              stepUpdate('image_execute', 'failure', null, (error) => {
                if (error) reject(error);
                else {
                  container.logs({
                    follow: true,
                    ...
                    // do not wait for stream end
                    Job.updateOne({ id: job_id }, {
                      $set: fields,
                      $push: { ['steps.image_execute.text']: '[error during image execution]' }
                    }, (error) => {
                      if (error) reject(error);
                      else reject(new Error('Received non-zero status code "' + data.StatusCode + '" from container'));
                    });
                  });
                }
              });
