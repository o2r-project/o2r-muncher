...
const Dns = require('../lib/model/dns');
...
function Executor(jobId, compendium) {
  ...
  this.executeImage = (passon) => {
    return new Promise((fulfill, reject) => {
      ...
      debug('[%s] passen object: %O', this.jobId, passon);
      ...
      this.updateStep('image_execute', 'running', '[started image execution]', (err) => {
        ...
        Dns.findOne({journal: this.compendium.publisher}, (err, dnsConf) => {
          let create_options;
          if (err || !dnsConf) {
            debug('[%s] Could not find DNS Server for this Compendium, setting NetworkDisables: true', job_id);
            create_options = Object.assign(
            ...
            debug('[%s] Found DNS Server for this Compendium, setting DNS IP: %s', job_id, dnsConf.ip);
            create_options = Object.assign(
                config.bagtainer.docker.create_options,
                {
                  name: 'muncher_job_' + this.jobId,
