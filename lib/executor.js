 ...
 * @param {string} jobBasePath - The path where all compendia are stored
 * @param {string} compendium - The compendium
 * @param {string} compendiumPath - The path where the compendium this job belongs to is stored
 */
function Executor(jobId, compendium) {
  ...
  copyFileFromJobToCompendium = function (file, job_id, compendium_id, isBag, callback) {
    ...
    try {
      debug('[%s] copy file %s from job %s to compendium %s (isBag? %s)', job_id, file, job_id, compendium_id, isBag);
  ...
  this.createAndLoadConfigurationFile = (passon) => {
    return new Promise((fulfill, reject) => {
            ...
            if (err) reject(err);
              ...
              yamlWriter(configFilePathJob, configuration, function (error) {
                if (error) {
                  ...
                  this.updateStep('generate_configuration', 'failure', 'error writing configuration file to job directory', (err) => {
                    if (err) reject(err);
                    else reject(new Error('error writing configuration file: ' + error.message));
                  });
                } else {
          ...
          fs.open(manifestFile, 'r+', (error) => {
            if (error) {
              ...
              fs.writeFile(manifestFile, 'FROM ubuntu\nCMD ["uname", "-a" ]', (err) => {
                ...
                this.updateStep('generate_manifest', 'success', 'created manifest file', (err) => {
                  if (err) reject(err);
  ...
  this.execute = () => {
    ...
    return this.begin({})
      ...
      .then(this.createAndLoadConfigurationFile)
