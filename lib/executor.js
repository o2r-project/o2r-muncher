          ...
          bag.validate(config.bagit.validateFast)
            .then(res => {
              ...
              if (config.bagit.failOnValidationError.execute) {
                ...
                this.updateStep('validate_bag', config.bagit.stepResultAfterValidationError, e.message, (err) => {
                  if (err) reject(err);
                  else fulfill(passon);
                });
              }
        ...
        docker.run(passon.imageTag, [], stdStream, create_options, start_options, (err, data, container) => {
          ...
          if (err) {
            ...
            if (data.StatusCode === 0) {
              stepUpdate('image_execute', 'success', null, (err) => {
                if (err) reject(err);

                Job.update({ id: job_id }, { 'steps.image_execute.statuscode': data.StatusCode }, (error) => {
                  if (error) reject(error);
              ...
              container.logs({
                ...
                stepUpdate('image_execute', 'failure', null, (err) => {
                  if (err) reject(err);
            ...
            .then(function (resolveMetadata) {
              ...
              stepUpdate('check', 'success', 'Check successful', (error) => {
                if (error) {
                  ...
                  let fields = {};
                  fields['steps.check'] = passon.check;
              ...
              this.updateStep('check', 'failure', JSON.stringify(rejectMetadata.errors), (error) => {
                if (error) {
                  debug('Error during check step update: %s', error.message);
                  reject(error);
                } else {
