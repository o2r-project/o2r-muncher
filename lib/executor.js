...
function Executor(jobId, compendium) {
  ...
  this.updateStep = (step, status, text, cb) => {
    ...
    switch (status) {
      case 'skipped':
        let d = new Date();
        fields['steps.' + step + '.start'] = d;
        fields['steps.' + step + '.end'] = d;
        break;
      case 'running':
      ...
      this.updateStep('image_execute', 'running', null, (err) => {
        ...
        let create_options = Object.assign(
          config.bagtainer.docker.create_options,
          {
            ...
            HostConfig: {
              Binds: [passon.jobPath + ':' + config.bagtainer.mountLocationInContainer],
              AutoRemove: true
            }
          }
        );
        ...
        if (error) {
          ...
          checker(checkerConfig)
            .then(function (resolveMetadata) {
              debug('[%s] Check succeeded: %s', job_id, resolveMetadata.checkSuccessful);
              passon.check = resolveMetadata;
              if (checkerConfig.saveDiffHTML) {
                passon.check.display = {};
                passon.check.display.diff = config.api.resource.job + job_id + config.api.sub_resource.data + 'diffHTML.html';
              }
              ...
              if (resolveMetadata.checkSuccessful) {
                ...
                stepUpdate('check', 'failure', 'Check failed', (error) => {
                  if (error) {
                    debug('Error during check step update: %s', error.message);
                    reject(error);
                  } else {
                    ...
                    Job.update({ id: job_id }, fields, (err) => {
                      if (err) {
                        debug('Error during check step fields: %s', err.message);
                        reject(err);
                      } else {
              ...
              if (checkerConfig.saveDiffHTML) {
                passon.check.display = {};
                passon.check.display.diff = config.api.resource.job + job_id + config.api.sub_resource.data + config.checker.display_file_name_html;
              }
          ...
          if (passon) {
            Promise.all([
              new Promise((fulfill, reject) => {
                if (passon.imageTag && !config.bagtainer.locateImageByTag) {
