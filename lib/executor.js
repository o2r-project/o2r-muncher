  ...
  this.packData = (passon) => {
    return new Promise((fulfill, reject) => {
      ...
      this.updateStep('image_prepare', 'running', null, (err) => {
        if (err) reject(err);
        if (!this.bagtainer.data) {
          this.updateStep('image_prepare', 'failure', 'no data found in passed object', (err) => {
            if (err) reject(err);
            reject(new Error('no data found in passed object'));
          });
        }
        else if (this.bagtainer.data.container) {
          ...
          this.updateStep('image_prepare', 'failure', 'no container-dir specified', (err) => {
            if (err) reject(err);
            reject(err);
          });
        }
