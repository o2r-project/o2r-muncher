...
const defaults = require('../config/erc-default');

const Job = require('../lib/model/job');
const jobIsBag = require('../lib/bagit').jobIsBag;

const steps = ['validate_bag', 'validate_compendium', 'image_prepare', 'image_build', 'image_execute', 'cleanup'];
...
function Executor(jobId, basePath) {
      ...
      Job.findOne({ id: this.jobId }, (err, job) => {
        if (err) {
          ...
          for (let step of steps) {
            ...
            if (job.steps[step].status === 'failure') {
              finalStatus = 'failure';
  ...
  this.detectBag = (passon) => {
    return new Promise((fulfill, reject) => {
      passon.isBag = jobIsBag(passon.id);
      fulfill(passon);
    });
  }
  ...
  this.validateBag = (passon) => {
    return new Promise((fulfill, reject) => {
      if (!passon.isBag) {
        debug('[%s] Not a bag, not attempting validation.', passon.id);
        fulfill(passon);
      } else if (config.bagtainer.validateBagBeforeExecute) {
        debug('[%s] Load and validate bag', passon.id);
        ...
        this.updateStep('validate_bag', 'running', null, (err) => {
          ...
          bag.validate(config.bagit.validateFast)
            .then(res => {
              debug('[%s] Bag is valid!', passon.id);
              this.updateStep('validate_bag', 'success', res, (err) => {
                if (err) {
                  debug('[%s] error updating step: %s', passon.id, err);
                  reject(err);
                  return;
                }
              ...
              this.updateStep('validate_bag', 'failure', e.message, (err) => {
                ...
                if (config.bagit.failOnValidationError.execute) {
                  debug('[%s] Bag invalid, failing this step: %s', passon.id, e);
                  reject(e);
                } else {
                  debug('[%s] Bag invalid but _not_ failing this step. The error was: %s', passon.id, e);
                  fulfill(passon);
                }
              });
            });
        ...
        debug('[%s] Bag validation disabled', passon.id);
  ...
  this.validateCompendium = (passon) => {
    return new Promise((fulfill, reject) => {
      if (config.bagtainer.validateCompendiumBeforeExecute) {
        debug('[%s] validating compendium %s', this.jobId, this.bagtainer.id);
        this.updateStep('validate_compendium', 'running', null, (err) => {
          if (err) reject(err);
          try {
            let configFilePath;
            if (passon.isBag) {
              configFilePath = path.join(this.basePath, this.jobId, config.bagtainer.payloadDirectory, config.bagtainer.configFile);
            } else {
              configFilePath = path.join(this.basePath, this.jobId, config.bagtainer.configFile);
            }

            this.bagtainer = yaml.load(configFilePath);
            debug('Parsed ERC configuration from file %s: %s', configFilePath, JSON.stringify(this.bagtainer));
            ...
            //else if (!this.bagtainer.execution.command) {
            //  this.updateStep('validate_compendium', 'failure', 'execution command required in config file', (err) => {
            //    if (err) reject(err);
            //    reject(new Error('"command" required in config file'));
            //  });
            //}

            if (!this.bagtainer.spec_version) {
              valid = false;
              if (config.bagtainer.failOnValidationError) {
            ...
            } else if (config.bagtainer.supportedVersions.indexOf(this.bagtainer.spec_version.toString()) == -1) {
              valid = false;
              let msg = '"version" ' + this.bagtainer.spec_version + ' is not supported (' +
        ...
        debug('[%s] Compenidum validation disabled', passon.id);
    ...
    return new Promise((fulfill, reject) => {
      ...
      this.updateStep('image_prepare', 'running', null, (err) => {
        ...
        if (this.bagtainer.containerfile) {
          containerfile = passon.bagpath + this.bagtainer.containerfile;
        } else {
          containerfile = passon.bagpath + defaults.containerfile;
        }
        ...
        try {
          ...
          if (containerfileStats.isFile()) {
            ...
            let fullpath = passon.bagpath + defaults.payloadDirectory;
   ...
   */
  this.execute = () => {
    debug('[%s] Executing for path %s', this.jobId, this.bagpath);

    return this.begin({ id: this.jobId, bagpath: this.bagpath })
      .then(this.detectBag)
