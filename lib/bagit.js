...
// TODO integrate validation functions from muncher and loader
...
jobIsBag = function (job_id) {
    ...
    try {
        ...
        debug('[%s] Could not find bag detection file for job, NOT a bag: %s', job_id, err.message);
        return false;
    }
}

function validateBagAlt(passon) {
    return new Promise((fulfill, reject) => {
        if(!passon.isBag) {
        ...
        bag
            ...
            .then((res) => {
                debug('[%s]] bag is valid: %o', passon.id, res);
...
module.exports.validateBagAlt = validateBagAlt;
