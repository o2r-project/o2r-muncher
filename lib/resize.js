...
exports.resize = (localPath, size, cb) => {
  ...
  switch (extension) {
    ...
    case 'jpeg':
    case 'bmp':
      debug('Resize %s as _image_ to %s', localPath, size);
    ...
    case 'markdown':
    case 'csv':
      debug('Resize %s as _text_ to %s', localPath, size);
...
function truncateText(localPath, size, extension, cb) {
  ...
  if (!isNaN(parseInt(size)) && size > 0) {
    var cached = path.join(c.fs.cache, hashName(localPath, size) + '.' + extension);
    debug('Truncating text file %s to %s lines using tmp file %s', localPath, size, cached);

    fs.access(cached, (err) => {
      if (err) {
        ...
        rl.on('line', (line) => {
          ...
          if (readLines < size) {
            ...
            try {
              ...
              debug('Error appending to chached file: %o', e);
              fs.access(cached, (e2) => {
                if (!e2) {
...
function resizeImage(localPath, size, extension, cb) {
  ...
  if (!isNaN(parseInt(size)) && size > 0) {
    ...
    let cached = path.join(c.fs.cache, hashName(localPath, size) + '.' + extension);
